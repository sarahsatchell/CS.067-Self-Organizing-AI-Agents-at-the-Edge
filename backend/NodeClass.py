...
class Node:
    ...
    def __init__(self, port: int, name: str, agent_id: int,
        ...
        self.aco_local_map:     Dict[Tuple[int, int], Set[Tuple[int, int]]] = {}
        # pheromone_map: node → current pheromone level (ACO trail)
        self.pheromone_map: Dict[Tuple[int, int], float] = {}

        self.aco_frontiers:       List[Tuple[int, int]]       = []
        self.aco_target_frontier: Optional[Tuple[int, int]]   = None

        # Map frontier coord → agent_id that claimed it (lowest ID wins)
        self.aco_claimed_frontiers: Dict[Tuple[int, int], int] = {}

        # ── Position ──────────────────────────────────────────────────── #
        self.aco_current_position: Optional[Tuple[int, int]] = None
        ...
        self.aco_stuck_counter:    int              = 0
        self.aco_recent_wall_hits: Set[Tuple[int, int]] = set()

        # ── Path & step tracking ──────────────────────────────────────── #
        self.aco_agent_path: List[Tuple[int, int]] = []
        self.aco_num_steps:  int = 0

        # ── Stats for completion display ──────────────────────────────── #
        self.aco_total_wall_hits:         int            = 0
        self.aco_tiles_discovered_directly: int          = 0
        self.aco_ticks_elapsed:           int            = 0
        self.aco_maps_merged:             int            = 0
        self.aco_maze_complete:           bool           = False
        self.aco_reached_goal:            bool           = False
        self.aco_goal_tick:               Optional[int]  = None
    ...
    def set_initial_position(self, position: Tuple[int, int]):
        ...
        self.aco_current_position = position
        self.aco_local_map[position] = set()
        self.aco_agent_path = [position]
        self.aco_tiles_discovered_directly = 1   # starting tile counts as discovered
    ...
    def _select_best_frontier_aco(self) -> Optional[Tuple[int, int]]:
        ...
        """
        if not self.aco_frontiers:
            return None

        # Prefer unclaimed frontiers; fall back to all frontiers if needed
        candidates = [f for f in self.aco_frontiers if f not in self.aco_claimed_frontiers]
        if not candidates:
            candidates = list(self.aco_frontiers)
        if not candidates:
            return None

        scores: List[float] = []
        for frontier in candidates:
            dist = self._get_bfs_distance(self.aco_current_position, frontier)
        ...
        while queue:
            ...
            for dx, dy in directions:
                ...
                if neighbor not in visited:
                    if neighbor in self.aco_local_map or neighbor == pos2:
    ...
    def _find_next_step_bfs(self,
        ...
        if target == self.aco_current_position:
            return None

        queue   = deque([(self.aco_current_position, [self.aco_current_position])])
        visited = {self.aco_current_position}
        directions = [(-1, 0), (1, 0), (0, -1), (0, 1)]
        ...
        while queue:
            ...
            for dx, dy in directions:
                neighbor = (cx + dx, cy + dy)
                if neighbor not in visited:
                    if neighbor in self.aco_local_map or neighbor == target:
                        visited.add(neighbor)
    ...
    def _move_toward_target(self) -> Tuple[int, int]:
        ...
        """
        if not self.aco_target_frontier or not self.aco_current_position:
            return self.aco_current_position

        if self.aco_current_position not in self.aco_local_map:
            self.aco_local_map[self.aco_current_position] = set()

        next_step = self._find_next_step_bfs(self.aco_target_frontier)

        if next_step is None:
            print(f"{self.name} cannot reach target {self.aco_target_frontier}. Dropping target.")
            self.aco_target_frontier = None
            self.aco_stuck_counter += 1
            if self.aco_stuck_counter > 5 and self.aco_frontiers:
                self.aco_target_frontier = random.choice(self.aco_frontiers)
                self.aco_stuck_counter = 0
            return self.aco_current_position
    ...
    def _scan_neighbors(self,
        ...
        """
        if not self.aco_current_position:
            return []

        new_frontiers: List[Tuple[int, int]] = []
        cx, cy = self.aco_current_position
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if self._is_wall(cell):
                if (nx, ny) not in self.aco_recent_wall_hits:
                    self.report_wall_hit((nx, ny))
                    self.aco_recent_wall_hits.add((nx, ny))
                    self.aco_total_wall_hits += 1
                    if len(self.aco_recent_wall_hits) > 10:
                        self.aco_recent_wall_hits.clear()
                continue

            if (nx, ny) not in self.aco_local_map:
                if (nx, ny) not in self.aco_frontiers:
                    self.aco_frontiers.append((nx, ny))
    ...
    def tick(self, local_grid_view: List[List[int]]):
        ...
        self.aco_frontiers = [f for f in self.aco_frontiers if f not in self.aco_local_map]

        # Clear reached target
        if self.aco_target_frontier == self.aco_current_position:
            self.aco_target_frontier = None

        # ANTI-LIVELOCK: yield target if a lower-ID agent has claimed it
        if self.aco_target_frontier and self.aco_target_frontier in self.aco_claimed_frontiers:
            owner_id = self.aco_claimed_frontiers[self.aco_target_frontier]
            if owner_id < self.agent_id:
                print(f"{self.name} yielding frontier {self.aco_target_frontier} "
                      f"to Agent_{owner_id} (lower ID)")
                self.aco_target_frontier = None

        # ── 2. SCAN ───────────────────────────────────────────────────── #
        new_frontiers = self._scan_neighbors(local_grid_view)
        new_nodes     = list(self.aco_local_map.keys())

        # ── 3. BROADCAST ─────────────────────────────────────────────── #
        self._broadcast_map_update(new_nodes, new_frontiers)

        # Broadcast pheromone trails every N ticks to avoid flooding peers
        if self.aco_ticks_elapsed % self.PHEROMONE_BROADCAST_INTERVAL == 0:
        ...
        # ── 5. DECIDE (ACO) ───────────────────────────────────────────── #
        if self.aco_target_frontier is None:
            selected = self._select_best_frontier_aco()
            if selected:
                self.aco_claimed_frontiers[selected] = self.agent_id
                self.aco_target_frontier = selected
        ...
        if self.aco_target_frontier:
            new_pos = self._move_toward_target()
            if new_pos != self.aco_current_position:
                prev_pos              = self.aco_current_position
                self.aco_current_position = new_pos
                self.aco_num_steps       += 1
                self.aco_agent_path.append(new_pos)

                # Register newly visited tile in local_map
                if self.aco_current_position not in self.aco_local_map:
                    self.aco_local_map[self.aco_current_position] = set()
                    self.aco_tiles_discovered_directly += 1

                # ── ACO: Deposit pheromone on the new tile ─────────────── #
                self._deposit_pheromone(self.aco_current_position)
                ...
                if new_pos == self.aco_target_frontier:
                    print(f"{self.name} reached target frontier {self.aco_target_frontier}")
                    if self.aco_target_frontier not in self.aco_local_map:
                        self.aco_local_map[self.aco_target_frontier] = set()
                    self.aco_target_frontier = None
    ...
    def _process_merge_packet(self, payload: Dict):
        ...
        for node in payload.get("nodes", []):
            ...
            if node_tuple not in self.aco_local_map:
                self.aco_local_map[node_tuple] = set()

        for frontier in payload.get("frontiers", []):
            frontier_tuple = tuple(frontier) if isinstance(frontier, list) else frontier
            if frontier_tuple not in self.aco_frontiers:
                self.aco_frontiers.append(frontier_tuple)

        self.aco_maps_merged += 1
    ...
    def _process_claim_packet(self, payload: Dict):
        ...
        if frontier:
            ...
            if frontier_tuple not in self.aco_claimed_frontiers:
                self.aco_claimed_frontiers[frontier_tuple] = sender_id
                print(f"{self.name} recorded claim from {sender} "
                      f"(ID {sender_id}): {frontier_tuple}")
            elif sender_id < self.aco_claimed_frontiers[frontier_tuple]:
                old_owner = self.aco_claimed_frontiers[frontier_tuple]
                self.aco_claimed_frontiers[frontier_tuple] = sender_id
    ...
    def report_wall_hit(self, wall_position: Tuple[int, int]):
        ...
        self.send_activity_log("agent_wall_hit", {
            "position":      list(self.aco_current_position) if self.aco_current_position else [0, 0],
    ...
    def _calculate_optimal_path_distance(self, goal: Tuple[int, int],
        ...
        """
        if not self.aco_agent_path:
            return 0

        start_pos  = self.aco_agent_path[0]
        rows       = len(maze)
    ...
    def get_agent_stats(self, goal: Tuple[int, int],
        ...
        """
        total_explored = len(self.aco_local_map)
        ...
        self_claimed_frontiers = len(
            [f for f in self.aco_claimed_frontiers
             if self.aco_claimed_frontiers[f] == self.agent_id]
        )
        unique_tiles_walked = len(set(self.aco_agent_path))
        redundant_steps     = max(0, self.aco_num_steps - (unique_tiles_walked - 1))
        exploration_rate    = (
            round(self.aco_tiles_discovered_directly / self.aco_num_steps, 3)
            if self.aco_num_steps > 0 else 0
        )
        ...
        return {
            ...
            "tiles_discovered_directly": self.aco_tiles_discovered_directly,
            "tiles_learned_from_peers":  total_explored - self.aco_tiles_discovered_directly,
            "steps_taken":               self.aco_num_steps,
            "unique_tiles_walked":       unique_tiles_walked,
            "redundant_steps":           redundant_steps,
            "efficiency_percentage":     round(
                ((unique_tiles_walked - 1) / self.aco_num_steps * 100)
                if self.aco_num_steps > 0 else 0, 1
            ),
            "exploration_rate":          exploration_rate,
            "remaining_frontiers":       remaining_frontiers,
            "frontiers_claimed_by_agent": self_claimed_frontiers,
            "walls_boundaries_hit":      self.aco_total_wall_hits,
            "maps_merged_from_peers":    self.aco_maps_merged,
            "ticks_elapsed":             self.aco_ticks_elapsed,
            "reached_goal":              self.aco_reached_goal,
            "goal_tick":                 self.aco_goal_tick,
            # ACO stats
    ...
    def get_agent_path(self):
        return self.aco_agent_path

    def get_agent_steps(self):
        return self.aco_num_steps